...
function getAge(age) {
  ...
  try {
    if (age > 0) {
      ...
      throw Object.assign(new Error("Вік не може бути менше 0!"), { name: "AgeError" });
    }
  } catch (err) {
...
function decodeURIComponentWrapper(encodedString) {
  ...
  try {
    let decodedURI = decodeURIComponent(encodedString);
    return decodedURI;
  } catch (err) {
    if (err instanceof URIError) {
      return `Помилка декодування ${err.message}`;
    } else {
      return err.message;
    }
  }
...
function validateUser(user) {
  ...
  try {
    ...
    if (!user.name) {
      throw new Error("Імя користувача не вказано!");
    }
    if (!user.email) {
    ...
    console.log(`${err.message} ${JSON.stringify(user)}`);
